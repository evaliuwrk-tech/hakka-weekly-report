/**
 * 週報填報系統 — Google Apps Script 後端（多組 · 每組獨立試算表）
 * ------------------------------------------------------------
 *  1. doPost：(a) 網頁填報 → 依 groupKey 寫進「該組自己的試算表」（週次+姓名 覆蓋）
 *            (b) LINE Webhook → 把最近群組 groupId 存進 LAST_GROUP_ID
 *  2. doGet ：看板用 JSONP，依 ?group=key 讀「該組自己的試算表」（不含內文）
 *  3. sendOverdueReminder：每週五 16:30 觸發器自動跑，逐組把逾期名單推播到各組 LINE 群
 *
 *  ★ 每組一張獨立試算表：GROUPS[i].sheetId 指定；留空/不填 = 用「本專案綁定的那張表」。
 *    → 哈客組沿用綁定的「哈客組週報登記」(不填 sheetId)；來上客組寫進自己的新表。
 *  ★ 改動此檔後務必重新部署；首次會要求「授權存取其他試算表」，請允許。
 */

/* ===== 需與 config.js 一致；sheetId 只在後端 ===== */
var FORM_URL = 'https://evaliuwrk-tech.github.io/hakka-weekly-report/';
var GROUPS = [
  { key: 'hakka', name: '哈客組', sheetId: '',   // 空 = 用綁定的「哈客組週報登記」
    members: ['劉怡君（組長）', '涂政強', '黃敏琪', '劉美君'] },
  { key: 'lsk',   name: '來上客組', sheetId: '1T2g4mMH90k7T28Dse1bOehSxm2ER-jeBA3Bvm8xtZxA',
...
function groupByKey_(key) { for (var i = 0; i < GROUPS.length; i++) if (GROUPS[i].key === key) return GROUPS[i]; return null; }

// 取得某組自己的 Submissions 工作表（各組一張獨立試算表）
function getSheetForGroup_(g) {
  var ss = (g && g.sheetId) ? SpreadsheetApp.openById(g.sheetId) : SpreadsheetApp.getActiveSpreadsheet();
...
function doPost(e) {
  ...
  // (a) 網頁填報 → 寫進該組自己的表（依 週次+姓名 覆蓋）
  var lock = LockService.getScriptLock();
  lock.tryLock(15000);
  try {
    var g = groupByKey_(data.groupKey) || GROUPS[0];
    var sh = getSheetForGroup_(g);
...
/* ============================ doGet（看板，依 ?group=key 讀該組的表） ============================ */
function doGet(e) {
  var key = (e && e.parameter && e.parameter.group) || '';
  var g = groupByKey_(key);
  var rows = [];
  try {
    if (g) {
      var sh = getSheetForGroup_(g), last = sh.getLastRow();
      if (last > 1) {
  ...
  var out = JSON.stringify({ ok: true, ver: 'multi-2', group: key, rows: rows });
...
function missingForGroup_(g) {
  ...
  var sh = getSheetForGroup_(g), submitted = {}, last = sh.getLastRow();
  if (last > 1) {
    var vals = sh.getRange(2, 2, last - 1, 3).getValues();
...
// 檢查各組試算表是否可正常存取（設定後跑一次確認權限 OK）
function testSheetsAccess() {
  GROUPS.forEach(function (g) {
    try { var sh = getSheetForGroup_(g); Logger.log('✓ ' + g.name + ' 試算表可存取，目前列數=' + sh.getLastRow()); }
    catch (e) { Logger.log('✗ ' + g.name + ' 無法存取：' + e); }
  });
}
